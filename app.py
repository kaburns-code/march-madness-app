...
def load_data():
    ...
    try:
        ...
        # Clean the weight column just in case there are spaces/strings
...
def set_matchup(team1, team2):
    ...
    st.set_query_params(scroll="top") # A simpler way to trigger a refresh/scroll
...
st.write("Calculates odds based on Season Stats + Active Injury Reports.")
...
if team_a_name and team_b_name:
    ...
    # Final Calculation (Base + Modifier - Team A Injuries + Team B Injuries)
    ...
    if not match_inj.empty:
        st.warning("⚠️ **Injury Impact Alert:** Probabilities have been shifted.")
...
st.divider()
st.header("🧬 Custom Round of 32 Matchups")
for r in regions:
    with st.expander(f"Region: {r}"):
        for p in ["A", "B", "C", "D"]:
            # Standard Pairing Logic: Matchups in Pod A play each other, etc.
            # In Pod A, it's (1v16) vs (8v9). In the code, these use s1=1 and s1=8.
            s_nums = {"A":[1,8], "B":[5,4], "C":[6,3], "D":[7,2]}[p]
